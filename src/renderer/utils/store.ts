...
export const useAppStore = () => {
  ...
  const sortedScrums = useMemo(() => [ ...scrums].sort((a,b) => b.id - a.id), [scrums])
  ...
  const setAlwaysOnTop = (alwaysOnTop: boolean) => {
    mergeSettings({'alwaysOnTop': alwaysOnTop})
  ...
  const addTask = (task: Omit<TaskData, 'id' | 'finished' | 'childTasksId'>) => {
  ...
  const addTaskToScrum = (scrumId: number, task: Parameters<typeof addTask>[0]) => {
    const scrumIdx = scrumIdIdxMap.get(scrumId)
    if (scrumIdx === undefined) return;
    const tasksId = [0, ...tasks.map(t => t.id)]
    const newTaskId = Math.max(...tasksId) + 1
  ...
  return {store, setStore, setAlwaysOnTop,
    ...
    addScrum, scrums, tasks, addTaskToScrum,
    getTaskById, removeTaskFromScrum, toggleFinishTask,
    sortedScrums, setTaskNow, setPrevNote, setBlockNote
  }
}


export const useScrum = (scrumId: number) => {
  const atomScrum = useMemo(() => atom((get) => (get(atomStore)?.scrums ?? []).find(s => s.id === scrumId)),[scrumId])
